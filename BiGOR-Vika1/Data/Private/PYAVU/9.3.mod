...
<P>При <Term Type="Out">простом связанном хранении</Term> каждый элемент списка представляет собой структуру nd, состоящую из двух элементов: val &#x2014; предназначен для хранения элемента списка, n &#x2014; для указателя на структуру, содержащую следующий элемент списка. На первый элемент списка указывает указатель dl. Для всех операций над списком используется описание:</P>
