...
<P>Когда сумма чисел от 1 до а становится нечетной, оператор continue передает управление на очередную итерацию цикла <Operator>for,</Operator> не выполняя операторы обработки четных сумм. Оператор <Operator>continue,</Operator> как и <Term Type="In">оператор break</Term>, прерывает самый внутренний из объемлющих его циклов.</P>
</Modul>
