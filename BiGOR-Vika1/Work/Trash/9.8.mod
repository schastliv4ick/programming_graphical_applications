...
<P>Например, для начального списка <Operator>B=&lt; 20,-5,10,8,7 &gt;</Operator> имеем: </P>
...
<P>Здесь оба списка В и В&apos; размещаются в массиве s, причем список В занимает часть s c индексами от i до n, а B&apos; - часть s c индексами от m до i-1 (см. рис.1). При сортировке вставкой требуется Q=(n-m)*(n-m) сравнений и не требуется дополнительной памяти. </P>
